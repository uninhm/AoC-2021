...
class PairNum: Num {
    ...
    override PairNum find_explosion(int depth = 0) {
        if (cast(RegNum)this.left &&
             cast(RegNum)this.right &&
             depth >= 4)
            return this;
...
class RegNum: Num {
    ...
    override bool reduce_by_split() {
        if (this.value < 10)
            return false;
        PairNum new_num = new PairNum(
            new RegNum(this.value/2),
            new RegNum((this.value+1)/2),
            this.parent
        );
...
void main() {
    ...
    while (!line.empty()) {
        ans = new PairNum(ans, line.parse());
        ans.reduce();
        line = input.readln.chomp;
    }
    ans.magnitude.writeln;
}